...
async function fixNeo4jEntityIds() {
  ...
  const pgResults = execPostgres(`
    SELECT entity_id, user_id, created_at, title 
    FROM memory_units 
    ORDER BY created_at
  `);
  ...
  for (const line of pgResults) {
    ...
    try {
      ...
      const [entity_id, user_id, created_at, title] = parts;
      
      // Find matching Neo4j node by user_id, created_at, and title
      const neo4jQuery = `
        MATCH (m:MemoryUnit)
        WHERE m.user_id = '${user_id}' 
        AND m.created_at = '${created_at}'
        AND m.title = '${title.replace(/'/g, "\\'")}'
      ...
      if (result && result.includes('updated')) {
        successCount++;
        console.log(`✅ Updated entity_id for ${entity_id}`);
      } else {
        errorCount++;
        console.log(`❌ Failed to update entity_id for ${entity_id}`);
      }
    } catch (error) {
      errorCount++;
