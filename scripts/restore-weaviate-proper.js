...
// Helper function to make HTTP requests without shell escaping issues
...
async function restoreWeaviateData() {
  console.log('🔄 Starting proper Weaviate data restoration...');
  ...
  // Process objects in smaller batches to avoid timeouts
  const batchSize = 10;
    ...
    for (const obj of batch) {
      try {
        ...
        if (!obj.properties.sourceEntityId && !obj.properties.textContent) {
          console.log(`⏭️  Skipping status-only object ${obj.id}`);
          continue;
        }
