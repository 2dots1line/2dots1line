...
/**
 * Clear embedding cache for a user/quest (for debugging)
 */
export const clearEmbeddingCache = async (userId: string = 'dev-user', questId?: string): Promise<void> => {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Cache clear API error: ${response.status} ${response.statusText}`);
    }

    const result = await response.json();
    console.log('🔍 EntityLookup: Cache cleared:', result);
  } catch (error) {
    console.warn('🔍 EntityLookup: Cache clear failed:', error);
  }
};

/**
 * Get embedding for a text phrase using the shared embedding service
 * Falls back to nearText if embedding API is not available
 */
const getEmbedding = async (text: string, userId: string = 'dev-user', questId?: string): Promise<number[] | null> => {
  try {
    // Try to call the shared embedding service via API
    ...
    console.warn('Embedding API not available, will use nearText fallback:', error);
...
export const findSimilarEntitiesByKeyPhrases = async (
  ...
  threshold: number,
  limit: number = 50,
  userId: string = 'dev-user', // Add userId parameter
  questId?: string // Add questId parameter
): Promise<any[]> => {
  try {
    const allSimilarEntities: any[] = [];
    
    for (const phrase of phrases) {
      console.log(`🔍 EntityLookup: Generating embedding for key phrase: "${phrase}"`);
      
      // Get embedding from shared service (reuses HRT embeddings)
      const embedding = await getEmbedding(phrase, userId, questId);
      
      let similarityResponse;
      
      if (embedding) {
        // Use nearVector search with the generated embedding
        console.log(`🔍 EntityLookup: Using nearVector with ${embedding.length}-dimensional vector for "${phrase}"`);
        similarityResponse = await fetch('http://localhost:8080/v1/graphql', {
          method: 'POST',
        ...
        // Fallback to nearText search
        console.log(`🔍 EntityLookup: Using nearText fallback for "${phrase}"`);
        similarityResponse = await fetch('http://localhost:8080/v1/graphql', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      console.log(`🔍 EntityLookup: Vector similarity result for "${phrase}":`, similarityResult);
    ...
    console.log(`🔍 EntityLookup: Found ${uniqueEntities.length} unique entities from key phrase vectors`);
...
export const findConnectedEntitiesAndRelationships = async (
  ...
  console.log('🔍 EntityLookup: Finding connected entities and relationships via Neo4j graph traversal');
  ...
  try {
    ...
    console.log('🔍 EntityLookup: Neo4j connected entities result:', connectedResult);

    if (!connectedResult.success || !connectedResult.data) {
      console.warn('🔍 EntityLookup: Neo4j connected entities query failed');
    ...
    console.log('🔍 EntityLookup: Neo4j relationships result:', relationshipsResult);

    const relationships = relationshipsResult.success && relationshipsResult.data ? relationshipsResult.data : [];

    console.log('🔍 EntityLookup: Found connected entities and relationships:', {
    ...
    console.error('🔍 EntityLookup: Neo4j graph traversal error:', error);
    return { connectedEntityIds: [], relationships: [] };
  }
};
...
export const fetchEntityData = async (entityIds: string[]): Promise<{nodes: any[], edges: any[]}> => {
  try {
    ...
    console.log('🔍 EntityLookup: Batch API response:', {
    ...
    console.error('🔍 EntityLookup: Failed to fetch entity data:', error);
    return { nodes: [], edges: [] };
  }
};
...
export const performKeyPhraseLookup = async (
  ...
  console.log('🔍 EntityLookup: Starting key phrase lookup:', { keyPhrases, config, userId, questId });
  
  // Clear cache for debugging to ensure fresh embeddings
  await clearEmbeddingCache(userId, questId);
  ...
  try {
    ...
    const similarEntities = await findSimilarEntitiesByKeyPhrases(
      ...
      50, // limit
      userId, // Pass userId to ensure user-specific embeddings
      questId // Pass questId to ensure quest-specific embeddings
    );
    ...
    if (config.enableGraphHops) {
      const graphResult = await findConnectedEntitiesAndRelationships(
        semanticEntityIds, 
        userId, // Use the same userId for Neo4j queries
        config.graphHops
      );
    ...
    console.log('🔍 EntityLookup: Key phrase lookup completed:', {
      ...
      questId // Log questId for debugging
    });
    ...
    console.error('🔍 EntityLookup: Key phrase lookup failed:', error);
    return {
      nodes: [],
      edges: [],
      similarEntities: [],
