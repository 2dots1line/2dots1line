...
*   **Primary Queues:** `card-queue` and `graph-queue` for presentation layer events (replaces legacy `card-and-graph-queue`).
...
*   **Processing Model:** Events intended for both presentation concerns MUST be published to both `card-queue` and `graph-queue`.
...
*   **V11.0 Polling Mechanism:** GraphProjectionWorker implements intelligent polling for embedding readiness with timeout protection (max 15 retries = 30 seconds) to prevent indefinite hanging.
...
*   **V11.0 Polling Behavior:** GraphProjectionWorker checks embedding readiness before processing, with intelligent retry logic and timeout protection.
...
*   **DLQ Names:** `{queue-name}-dlq` (e.g., `card-queue-dlq`, `graph-queue-dlq`)
...
```typescript
// Publish to both queues to ensure card and graph workers both receive the event
await cardQueue.add('new_entities_created', payload, options);
await graphQueue.add('new_entities_created', payload, options);
```

### From InsightEngine:
```typescript
// Publish to both queues to ensure card and graph workers both receive the event
await cardQueue.add('cycle_artifacts_created', payload, options);
await graphQueue.add('cycle_artifacts_created', payload, options);

// Ontology updates target graph only
await graphQueue.add('graph_ontology_updated', payload, options);
```
